...
export class SubjectGradingBookTemplateDetailPage extends BasePage {
  ...
  // columnRowByName(name: string): Locator {
  //   return this.page.locator(
  //     `.dx-scrollable-content tbody[role="presentation"] tr.dx-data-row`
  //   ).filter({
  //     has: this.page.locator(`td[aria-colindex="1"] .dx-template-wrapper > div:first-child`, {
  ...
  readOnlyCheckboxInRow(code: string): Locator {
    return this.columnRowByCode(code).locator(
      'td input[type="checkbox"], td [class*="checkbox"], td [class*="readonly"]'
    ).first();
  }
